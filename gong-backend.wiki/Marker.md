...
`Path: ./app/controllers/marker_controller.rb`

* Get markers
```sh
Method: get_markers
Route: GET http://localhost:3000/marker/get_markers
Params: locationLat, locationLng, radius[, markerType, markerCategory]
* Optional params to pagination: page, [ limit(default:1)]
* Optional params to sector select: direction, angle
* Optional params to name filter: looklike (trái|phải|tốc độ)
Success:
{
    "success": true,
    "markers": [
...
Params: locationLat(Max/Min), locationLng(Max/Min)
* Optional params to filter: markerType, markerCategory
* Optional params to pagination: page, [ limit(default:1)]
* Optional params to name filter: looklike (trái|phải|tốc độ)
Success:
{
...
Params: type, id (type in ['EMPTY', 'REMOVE'])
* Optional params: new_name (required for type:NWRONG)
* Optional params: new_lat, new_lng (required for type:PWRONG),
                   description_addess, description_info_review, description_number....
* Optional params: new_speed (required for type:SWRONG)
Success:
{
   "success": true,
   "Message":"Success"
